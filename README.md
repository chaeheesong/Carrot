...
> **"우리 동네 중고거래 — GraphQL의 강점을 눈으로 확인하는 당근마켓 오마주 웹앱"**
...
* **📄 문서(.md) 기반 지시 — "단일 진실 공급원(SSOT)"**
  구두 지시 대신 `docs/기획서.md` · `docs/개발계획.md` · `env.md` 를 만들어
...
* **🔒 민감 파일 읽기 차단 규칙 (권한 통제)**
  ...
  **DB 비밀번호·API 키가 담긴 `.env` 를 Claude가 아예 읽지 못하도록** 원천 차단했습니다.
  비밀값은 앱 런타임만 사용하고 **AI의 컨텍스트에는 절대 들어가지 않도록** 통제했습니다.
