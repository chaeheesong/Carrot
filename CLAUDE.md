# 프로젝트 개요
동네장터 — 동네 기반 중고거래 GraphQL 웹 서비스 (당근마켓 오마주). 홈 피드 / 상품 상세 / 실시간 채팅이 핵심.
npm workspaces 모노레포: `apps/web`(Next.js 프론트) + `apps/server`(Apollo Server 백엔드). 두 워크스페이스가 GraphQL 타입을 공유.
...
- **🔒 `.env`는 절대 읽지 말 것** — `.claude/settings.json`의 `permissions.deny`에 `Read(**/.env)` 등록됨. 비밀값은 런타임만 사용, 값 존재 여부는 동작으로만 확인.
